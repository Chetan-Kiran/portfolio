          ...
          >
            ...
            <div className="bg-primary/5 p-8 md:p-12 border-b border-glass-border flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="space-y-4 max-w-2xl">
                <h2 className="text-4xl md:text-5xl font-black gradient-text tracking-tight">
                  {project.title}
                </h2>
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={onClose}
                  className="p-4 bg-primary/10 hover:bg-primary/20 rounded-2xl border border-primary/30 transition-all text-primary hover:text-primary-light"
                >
                  <X size={24} />
                </button>
              </div>
                    ...
                    {project.milestones?.map((m, i) => {
                      ...
                      return (
                        ...
                        >
                          {/* Connector Node */}
                          <div
                            className={`absolute left-4 md:left-1/2 -translate-x-1/2 w-8 h-8 rounded-full border-4 border-background z-10 transition-transform hover:scale-125 ${isProblem ? "bg-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.4)]" : "bg-primary shadow-[0_0_20px_rgba(59,130,246,0.4)]"}`}
                          />

                          {/* Content Card */}
                          <div
                            className={`w-full bg-white/5 border rounded-[2rem] p-8 shadow-xl backdrop-blur-md hover:bg-white/10 transition-all ${isProblem ? "border-orange-500/20" : "border-primary/20"}`}
                          >
                            <div className="flex items-center gap-3 mb-4">
                              <div
                                className={`p-2 rounded-lg ${isProblem ? "bg-orange-500/10 text-orange-400" : "bg-primary/10 text-primary"}`}
                              >
                                {isProblem ? (
                          ...
                          <div
                            className={`hidden md:block absolute top-1/2 -translate-y-1/2 ${isProblem ? "left-[calc(50%+16px)]" : "right-[calc(50%+16px)]"}`}
                          >
                            <ArrowRight
                              size={20}
                              className={
